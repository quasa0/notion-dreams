...
export function ReviewBoard({ initialEdits, initialRuns }: Props) {
  ...
  const [topBusy, setTopBusy] = useState<"refresh" | "trigger" | "seed" | "mingle" | null>(null);
  const [seedResult, setSeedResult] = useState<string | null>(null);
  const [seededPages, setSeededPages] = useState<SeededPage[]>([]);
  const [noticeExpiresAt, setNoticeExpiresAt] = useState<number | null>(null);
  ...
  async function triggerDream() {
    ...
    setTopBusy("trigger");
    setRuns((current) => [optimisticRun, ...current.filter((run) => run.key !== optimisticRun.key)]);
    setSelectedRunKey(optimisticRun.key);
    try {
      ...
      setRuns((current) => current.filter((run) => run.key !== optimisticRun.key));
  ...
  async function refreshDashboardData(waitForRanAt?: string) {
    ...
    setEdits(data.edits);
    setRuns((current) => {
      const merged = mergeRunsWithInitiating(data.runs, current);
      setSelectedRunKey((selected) => merged.some((run) => run.key === selected) ? selected : merged[0]?.key ?? null);
      return merged;
    });
  }
  async function seedFastclipPages() {
    ...
    try {
      ...
      setNoticeExpiresAt(Date.now() + 10000);
    } catch (error) {
      setSeededPages([]);
  ...
  async function minglePages() {
    ...
    try {
      ...
      setSeededPages(data.edited ?? []);
      setSeedResult(`Mingled ${data.edited.length} existing pages in Notion`);
      setNoticeExpiresAt(Date.now() + 10000);
    } catch (error) {
      setSeededPages([]);
      setSeedResult(error instanceof Error ? error.message : "Failed to mingle pages");
      setNoticeExpiresAt(Date.now() + 10000);
    } finally {
      setTopBusy(null);
    }
...
async function waitForDashboardRun(ranAt: string): Promise<DashboardData> {
  ...
  while (Date.now() < deadline) {
    if (latest.runs.some((run) => run.ran_at && new Date(run.ran_at).getTime() >= targetMs - 5000)) return latest;
