...
function ReportEmbed({run,url,isPublic,live}:{run:DreamRun;url?:string;isPublic:boolean;live:boolean}){return <div className="overflow-hidden rounded-xl border border-line bg-white"><div className="flex items-center justify-between gap-4 border-b border-line px-4 py-3"><div><div className="font-mono text-[10px] uppercase tracking-[0.14em] text-mute">Notion report</div>{!isPublic&&<p className="mt-1 text-[12px] text-mute">Rendered from Notion page data inside the dashboard. Open in Notion to view the original page.</p>}</div>{url&&<a href={url} target="_blank" rel="noreferrer" className="shrink-0 rounded-md border border-line bg-paper px-2.5 py-1.5 text-[12px] font-medium text-ink hover:bg-paper2">Open</a>}</div><NotionReportPreview pageId={run.report_page_id} live={live}/></div>}
